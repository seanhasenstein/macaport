...
import ProductSidebar from '../../../components/demo-store/ProductSidebar';
...
export default function Product({ item }: ProductProps) {
  ...
  const [color, setColor] = React.useState(item.colors[0].label);
  ...
  const handleSizeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (error && e.target.value !== 'DEFAULT') setError(undefined);
  ...
  const handleButtonClick = () => {
    ...
    if (sku && item)
      addItem({ ...item, id: sku.id, productId: item.id, color, size });
    setShowSidebar(true);
  };
  ...
  return (
    <Layout title={`${item.name} | Track & Field Apparel Order`}>
      <ProductStyles>
        <div className="wrapper">
          ...
          <div className="primary">
            ...
            <h3 className="price">{formatToMoney(item.price)}</h3>
          </div>
...
export const getServerSideProps: GetServerSideProps = async context => {
  ...
  // todo: handle when query.id isn't provided and when query.id is invalid
